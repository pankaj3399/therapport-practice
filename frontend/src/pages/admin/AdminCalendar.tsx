...
const TIME_OPTIONS_30MIN = (() => {
  ...
  for (let h = 8; h <= 21; h++) {
    options.push({ value: `${h.toString().padStart(2, '0')}:00`, label: `${h.toString().padStart(2, '0')}:00` });
    if (h <= 21) options.push({ value: `${h.toString().padStart(2, '0')}:30`, label: `${h.toString().padStart(2, '0')}:30` });
  }
  return options;
})();
...
type PractitionerOption = { id: string; label: string };
...
export const AdminCalendar: React.FC = () => {
  ...
  const [bookingType, setBookingType] = useState<'ad_hoc' | 'permanent_recurring' | 'free' | 'internal'>('ad_hoc');
    ...
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        if (res.data.success && res.data.rooms) {
          ...
          setSelectedRoomId((prev) => (res.data.rooms?.some((r) => r.id === prev) ? prev : res.data.rooms?.[0]?.id ?? null));
        }
      } catch {
        setRooms([]);
  ...
  const fetchPractitioners = useCallback(async () => {
    ...
    try {
      ...
      const data = res.data as { success?: boolean; data?: Array<{ id: string; firstName?: string; lastName?: string; email: string }> };
  ...
  useEffect(() => {
    if (bookingType === 'free' || bookingType === 'internal') {
      fetchPractitioners();
    }
  }, [bookingType, fetchPractitioners]);
  ...
  useEffect(() => {
    ...
    practitionerApi
      ...
      .then((res) => {
        ...
        if (res.data.success && typeof res.data.totalPrice === 'number') setQuotePrice(res.data.totalPrice);
        else setQuotePrice(null);
      })
      .catch(() => { if (!c.signal.aborted) setQuotePrice(null); })
      .finally(() => { if (!c.signal.aborted) setLoadingQuote(false); });
    return () => c.abort();
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      ...
      if (data.success && !data.paymentRequired && data.booking) {
        ...
        setCreateError('Payment required for this booking type when creating on behalf of a practitioner. Use free/internal with target practitioner for no charge.');
      } else {
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-1">
            View room usage, create bookings on behalf of practitioners, and modify or cancel bookings.
          </p>
        </div>
            ...
            {loadingCalendar ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse min-w-[400px] text-sm">
                  <thead>
                    <tr>
                      <th className="border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 p-1.5 text-left text-xs font-medium text-slate-600 dark:text-slate-400 w-[60px]">Time</th>
                      {calendarRooms.map((r) => (
                        <th key={r.id} className="border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 p-1.5 text-center text-xs font-medium text-slate-700 dark:text-slate-300">{r.name}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {Array.from({ length: 28 }, (_, i) => {
                      ...
                      return (
                        <tr key={i}>
                          <td className="border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-0.5 text-xs text-slate-500 dark:text-slate-400 align-top">{timeLabel}</td>
                          {calendarRooms.map((room) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="flex flex-col gap-1">
                                    {booking.bookerName && <span className="text-xs font-medium">{booking.bookerName}</span>}
                                    {booking.id && (
                            ...
                            return (
                              <td key={room.id} className="border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-0.5 min-h-[14px]" />
                            );
                          })}
                        </tr>
                      );
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-4 items-end">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-slate-600 dark:text-slate-400">
                  Price: {loadingQuote ? '—' : quotePrice != null ? `£${quotePrice.toFixed(2)}` : '—'}
                </span>
            ...
            {createSuccess && <output className="block text-sm text-green-600 dark:text-green-400" aria-live="polite">{createSuccess}</output>}
            {cancelError && <p className="text-sm text-red-600 dark:text-red-400" role="alert">{cancelError}</p>}
          </CardContent>
        </Card>
      </div>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <select
                  ...
                  onChange={(e) => setModifyBooking((p) => (p ? { ...p, roomId: e.target.value } : null))}
                  className="rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm"
                >
                  {calendarRooms.map((r) => (
              ...
              <div className="grid gap-2">
                ...
                <input
                  ...
                  min={todayDateString()}
                  max={maxBookingDateString()}
                  onChange={(e) => setModifyBooking((p) => (p ? { ...p, bookingDate: e.target.value } : null))}
                  className="rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm"
                />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div className="grid gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) => setModifyBooking((p) => (p ? { ...p, startTime: e.target.value } : null))}
                    className="rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm"
                  >
                    {TIME_OPTIONS_30MIN.map((o) => (
                      <option key={o.value} value={o.value}>{o.label}</option>
                ...
                <div className="grid gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) => setModifyBooking((p) => (p ? { ...p, endTime: e.target.value } : null))}
                    className="rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm"
                  >
                    {TIME_OPTIONS_30MIN.map((o) => (
                      <option key={o.value} value={o.value}>{o.label}</option>
              ...
              {modifyError && <p className="text-sm text-red-600 dark:text-red-400" role="alert">{modifyError}</p>}
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setModifyBooking(null)}>Cancel</Button>
